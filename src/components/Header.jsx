...
export default function Header() {
    ...
    return (
        <header className="sticky top-0 z-10 bg-brand-light border-b border-orange-100/50 shadow-sm">
            <div className="flex items-center justify-between p-3 md:p-4 md:px-8 max-w-screen-xl mx-auto gap-2">
                <Link to="/" className="flex items-center gap-2 md:gap-3 hover:opacity-80 transition-opacity min-w-0">
                    <div className="w-8 h-8 md:w-10 md:h-10 rounded-full bg-orange-100 overflow-hidden border-2 border-orange-200 shadow-sm shrink-0">
                        <img src="/logo.jpg" alt="Ammachi's Kitchen Logo" className="w-full h-full object-cover" />
                    </div>
                    <span className="text-lg sm:text-xl md:text-2xl font-serif font-bold text-brand-primary tracking-tight leading-none md:leading-tight truncate">
                        Ammachi's Kitchen
                    </span>
                </Link>

                <nav className="hidden md:flex items-center gap-2 md:gap-4 font-medium bg-white/60 backdrop-blur-md px-2 py-1.5 rounded-full shadow-sm border border-orange-100/50">
                    <Link
                        to="/"
